var Grad = function(image, classID, optNumColors) {

  this.imageData = this.toImageData(image);

  // Colors which do not catch the eye
  this.ignoredColors = [[255,255,255]];
...
// added this so we can make the colors darker/lighter if desired
...
Grad.prototype.createCSS = function(colors) {
  ...
  //for (var j = 0; j < this.browserPrexies.length; j++, s[j] = [""]) {
    ...
    for (var i=0; i<colors.length; i++) {
      ...
      color1 = shadeRGBColor("rgb(" + colors[i][0] + ", " + colors[i][1] + ", " + colors[i][2] + ")", -0.1)

      s += "linear-gradient(to " + oppDir + ", rgba(" +
          color1 + ",0) 0%, rgba(" +
          color1 + ",0.1) 100%), "
    }
  ...
  $("." + this.classID).css('background', s);

  //$("." + this.classID).css({color: "rgb(" + colors[1][0] + ", " + colors[1][1] + ", " + colors[1][2] + ")"})
